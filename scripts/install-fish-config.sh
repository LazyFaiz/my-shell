...
if [[ ${1:-} == --help ]]; then
  printf '%s\n' 'Usage: bash scripts/install-fish-config.sh [--config-only] [--astronvim] [--zellij] [--yazi] [--tealdeer] [--zoxide] [--starship] [--fzf] [--dust] [--duf] [--procs] [--delta]' \
...
config_only=0 astro=0 delta=0 zellij=0 yazi=0 tealdeer=0 zoxide=0 starship=0 fzf=0 dust=0 duf=0 procs=0
for arg in "$@"; do
  case "$arg" in
...
for tool in neovim zellij yazi tealdeer zoxide starship fzf dust duf procs; do
  selected=0
  case "$tool" in neovim) selected=$astro ;; zellij) selected=$zellij ;; yazi) selected=$yazi ;; tealdeer) selected=$tealdeer ;; zoxide) selected=$zoxide ;; starship) selected=$starship ;; fzf) selected=$fzf ;; dust) selected=$dust ;; duf) selected=$duf ;; procs) selected=$procs ;; esac
  if (( selected )); then
