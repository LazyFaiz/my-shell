...
if [[ ${1:-} == --help ]]; then
  echo 'Usage: bash scripts/install-tealdeer.sh'
  echo 'Install latest stable tealdeer from GitHub for Linux/macOS x86_64/ARM64.'
  exit 0
fi
...
curl -fsSL --retry 3 --connect-timeout 15 --max-time 120 \
  https://api.github.com/repos/tealdeer-rs/tealdeer/releases/latest -o "$work/release.json"
...
if [[ $system == linux ]]; then
  asset="tealdeer-linux-$arch-musl"
else
  asset="tealdeer-macos-$arch"
fi
if release_is_current "$version" "$asset" tldr; then
...
[[ "$url" == "https://github.com/tealdeer-rs/tealdeer/releases/download/$version/$asset" ]] || { echo 'Unexpected release URL.' >&2; exit 1; }
[[ "$digest" =~ ^sha256:[a-fA-F0-9]{64}$ ]] || { echo 'Release has no valid SHA-256 digest.' >&2; exit 1; }
INSTALL_STEP=archive-download
echo "Downloading tealdeer $version ($system/$arch)..."
curl -fL --retry 3 --connect-timeout 15 --max-time 600 "$url" -o "$work/tldr"
INSTALL_STEP=checksum
actual=$(checksum "$work/tldr")
...
target=$(mktemp -d "$HOME/.local/opt/tldr-$version-XXXXXXXX")
mkdir -p "$target/bin"
install -m 0755 "$work/tldr" "$target/bin/tldr"
"$target/bin/tldr" --version
printf '%s %s\n' "$version" "$asset" > "$target/.my-shell-release"
INSTALL_STEP=activate-entry
entry="$HOME/.local/bin/tldr"
...
if [[ -e "$entry" || -L "$entry" ]]; then
  backup_dir=$(mktemp -d "$HOME/.local/opt/tldr-entry-backup-XXXXXXXX")
  cp -a "$entry" "$backup_dir/tldr"
  echo "Previous command backed up: $backup_dir/tldr"
fi
# Stage the link on the same filesystem, then replace only the command entry.
stage=$(mktemp -d "$HOME/.local/bin/.tldr-link-XXXXXXXX")
ln -s "$target/bin/tldr" "$stage/tldr"
mv -f "$stage/tldr" "$entry"
rmdir "$stage"
