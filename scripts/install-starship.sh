...
if [[ ${1:-} == --help ]]; then
  echo 'Usage: bash scripts/install-starship.sh'
  echo 'Install latest stable starship from GitHub for Linux/macOS x86_64/ARM64.'
  exit 0
fi
...
github_latest_release starship/starship "$work/release.json"
...
asset="starship-$arch-$system"
if release_is_current "$version" "$asset" starship; then
...
[[ "$url" == "https://github.com/starship/starship/releases/download/$version/$asset.tar.gz" ]] || { echo 'Unexpected release URL.' >&2; exit 1; }
[[ "$digest" =~ ^sha256:[a-fA-F0-9]{64}$ ]] || { echo 'Release has no valid SHA-256 digest.' >&2; exit 1; }
INSTALL_STEP=archive-download
echo "Downloading starship $version ($system/$arch)..."
curl -fL --retry 3 --connect-timeout 15 --max-time 600 "$url" -o "$work/starship.tar.gz"
INSTALL_STEP=checksum
actual=$(checksum "$work/starship.tar.gz")
[[ "${actual%% *}" == "${digest#sha256:}" ]] || { echo 'SHA-256 mismatch; installation stopped.' >&2; exit 1; }
INSTALL_STEP=extract-archive
tar -xzf "$work/starship.tar.gz" -C "$work"
INSTALL_STEP=verify-binary
# Verify the downloaded binary before changing the active command.
"$work/starship" --version
mkdir -p "$HOME/.local/opt" "$HOME/.local/bin"
target=$(mktemp -d "$HOME/.local/opt/starship-$version-XXXXXXXX")
mkdir -p "$target/bin"
install -m 0755 "$work/starship" "$target/bin/starship"
"$target/bin/starship" --version
printf '%s %s\n' "$version" "$asset" > "$target/.my-shell-release"
INSTALL_STEP=activate-entry
entry="$HOME/.local/bin/starship"
...
if [[ -e "$entry" || -L "$entry" ]]; then
  backup_dir=$(mktemp -d "$HOME/.local/opt/starship-entry-backup-XXXXXXXX")
  cp -a "$entry" "$backup_dir/starship"
  echo "Previous command backed up: $backup_dir/starship"
fi
# Stage the link on the same filesystem, then replace only the command entry.
stage=$(mktemp -d "$HOME/.local/bin/.starship-link-XXXXXXXX")
ln -s "$target/bin/starship" "$stage/starship"
mv -f "$stage/starship" "$entry"
rmdir "$stage"
