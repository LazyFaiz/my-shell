...
if [[ ${1:-} == --help ]]; then
  echo 'Usage: bash scripts/install-duf.sh'
  echo 'Install latest stable duf from GitHub for Linux/macOS x86_64/ARM64.'
  exit 0
fi
...
github_latest_release muesli/duf "$work/release.json"
...
asset="duf_${version#v}_${system}_${arch}"
if release_is_current "$version" "$asset" duf; then
...
[[ "$url" == "https://github.com/muesli/duf/releases/download/$version/$asset.tar.gz" ]] || { echo 'Unexpected release URL.' >&2; exit 1; }
[[ "$digest" =~ ^sha256:[a-fA-F0-9]{64}$ ]] || { echo 'Release has no valid SHA-256 digest.' >&2; exit 1; }
INSTALL_STEP=archive-download
echo "Downloading duf $version ($system/$arch)..."
curl -fL --retry 3 --connect-timeout 15 --max-time 600 "$url" -o "$work/duf.tar.gz"
INSTALL_STEP=checksum
actual=$(checksum "$work/duf.tar.gz")
[[ "${actual%% *}" == "${digest#sha256:}" ]] || { echo 'SHA-256 mismatch; installation stopped.' >&2; exit 1; }
INSTALL_STEP=extract-archive
tar -xzf "$work/duf.tar.gz" -C "$work"
INSTALL_STEP=verify-binary
# Verify the downloaded binary before changing the active command.
"$work/duf" --version
mkdir -p "$HOME/.local/opt" "$HOME/.local/bin"
target=$(mktemp -d "$HOME/.local/opt/duf-$version-XXXXXXXX")
mkdir -p "$target/bin"
install -m 0755 "$work/duf" "$target/bin/duf"
"$target/bin/duf" --version
printf '%s %s\n' "$version" "$asset" > "$target/.my-shell-release"
INSTALL_STEP=activate-entry
entry="$HOME/.local/bin/duf"
...
if [[ -e "$entry" || -L "$entry" ]]; then
  backup_dir=$(mktemp -d "$HOME/.local/opt/duf-entry-backup-XXXXXXXX")
  cp -a "$entry" "$backup_dir/duf"
  echo "Previous command backed up: $backup_dir/duf"
fi
# Stage the link on the same filesystem, then replace only the command entry.
stage=$(mktemp -d "$HOME/.local/bin/.duf-link-XXXXXXXX")
ln -s "$target/bin/duf" "$stage/duf"
mv -f "$stage/duf" "$entry"
rmdir "$stage"
