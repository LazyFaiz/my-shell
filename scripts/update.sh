...
case ${1:---help} in
  --help|-h)
    printf '%s\n' 'Usage: bash scripts/update.sh [--shell zsh|fish] config|plugins|tools|all [neovim|zellij|yazi|tealdeer|zoxide|starship|fzf|dust|duf|procs|fish ...]' \
...
for tool in "$@"; do
  case "$tool" in neovim|zellij|yazi|tealdeer|zoxide|starship|fzf|dust|duf|procs|fish) ;; *) echo "Unknown tool: $tool" >&2; exit 2 ;; esac
done
...
if [[ $# == 0 ]]; then
  set -- neovim zellij yazi tealdeer zoxide starship fzf dust duf procs
