...
if [[ ${1:-} == --help ]]; then
  echo 'Usage: bash scripts/install-dust.sh'
  echo 'Install latest stable dust from GitHub for Linux/macOS x86_64/ARM64.'
  exit 0
fi
...
github_latest_release bootandy/dust "$work/release.json"
...
asset="dust-$version-$arch-$system"
if release_is_current "$version" "$asset" dust; then
...
[[ "$url" == "https://github.com/bootandy/dust/releases/download/$version/$asset.tar.gz" ]] || { echo 'Unexpected release URL.' >&2; exit 1; }
[[ "$digest" =~ ^sha256:[a-fA-F0-9]{64}$ ]] || { echo 'Release has no valid SHA-256 digest.' >&2; exit 1; }
INSTALL_STEP=archive-download
echo "Downloading dust $version ($system/$arch)..."
curl -fL --retry 3 --connect-timeout 15 --max-time 600 "$url" -o "$work/dust.tar.gz"
INSTALL_STEP=checksum
actual=$(checksum "$work/dust.tar.gz")
[[ "${actual%% *}" == "${digest#sha256:}" ]] || { echo 'SHA-256 mismatch; installation stopped.' >&2; exit 1; }
INSTALL_STEP=extract-archive
tar -xzf "$work/dust.tar.gz" -C "$work"
INSTALL_STEP=verify-binary
# Verify the downloaded binary before changing the active command.
"$work/$asset/dust" --version
mkdir -p "$HOME/.local/opt" "$HOME/.local/bin"
target=$(mktemp -d "$HOME/.local/opt/dust-$version-XXXXXXXX")
mkdir -p "$target/bin"
install -m 0755 "$work/$asset/dust" "$target/bin/dust"
"$target/bin/dust" --version
printf '%s %s\n' "$version" "$asset" > "$target/.my-shell-release"
INSTALL_STEP=activate-entry
entry="$HOME/.local/bin/dust"
...
if [[ -e "$entry" || -L "$entry" ]]; then
  backup_dir=$(mktemp -d "$HOME/.local/opt/dust-entry-backup-XXXXXXXX")
  cp -a "$entry" "$backup_dir/dust"
  echo "Previous command backed up: $backup_dir/dust"
fi
# Stage the link on the same filesystem, then replace only the command entry.
stage=$(mktemp -d "$HOME/.local/bin/.dust-link-XXXXXXXX")
ln -s "$target/bin/dust" "$stage/dust"
mv -f "$stage/dust" "$entry"
rmdir "$stage"
