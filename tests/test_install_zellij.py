...
SCRIPT = Path(__file__).resolve().parents[1] / 'scripts/install-zellij.sh'


class ZellijInstallerTests(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix='zellij-installer-test-')
        self.addCleanup(self.temp.cleanup)
        self.root = Path(self.temp.name)
        self.home = self.root / 'home with spaces'
        self.bin = self.root / 'mocks'
        self.bin.mkdir()
        self.entry = self.home / '.local/bin/zellij'
...
''')
    ...
    def fixture(self, system='Linux', machine='x86_64', prerelease=False,
        ...
        release = {'tag_name': 'v0.12.5', 'draft': False, 'prerelease': prerelease,
                   'assets': [{'name': name + '.tar.gz', 'digest': 'sha256:' + digest,
                    'browser_download_url': f'https://github.com/zellij-org/zellij/releases/download/v0.12.5/{name}.tar.gz'}]}
    ...
    def test_supported_platforms_and_repeat_install(self):
        ...
        backups = list((self.home / '.local/opt').glob('zellij-entry-backup-*/zellij'))
    ...
    def test_main_installer_zellij_option(self):
        self.fixture()
        ...
        result = subprocess.run(['bash', str(SCRIPT.with_name('install-config.sh')), '--zellij'],
