...
SCRIPT = Path(__file__).resolve().parents[1] / 'scripts/install-zoxide.sh'


class ZoxideInstallerTests(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix='zoxide-installer-test-')
        self.addCleanup(self.temp.cleanup)
        self.root = Path(self.temp.name)
        self.home = self.root / 'home with spaces'
        self.bin = self.root / 'mocks'
        self.bin.mkdir()
        self.entry = self.home / '.local/bin/zoxide'
...
''')
    ...
    def fixture(self, system='Linux', machine='x86_64', prerelease=False,
        ...
        name = f'zoxide-0.10.0-{arch}-{platform}'
        ...
        release = {'tag_name': 'v0.10.0', 'draft': False, 'prerelease': prerelease,
                   'assets': [{'name': name + '.tar.gz', 'digest': 'sha256:' + digest,
                    'browser_download_url': f'https://github.com/ajeetdsouza/zoxide/releases/download/v0.10.0/{name}.tar.gz'}]}
    ...
    def test_supported_platforms_and_repeat_install(self):
        ...
        backups = list((self.home / '.local/opt').glob('zoxide-entry-backup-*/zoxide'))
    ...
    def test_main_installer_zoxide_option(self):
        self.fixture()
        ...
        result = subprocess.run(['bash', str(SCRIPT.with_name('install-config.sh')), '--zoxide'],
