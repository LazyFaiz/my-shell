...
SCRIPT = Path(__file__).resolve().parents[1] / 'scripts/install-dust.sh'


class DustInstallerTests(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix='dust-installer-test-')
        self.addCleanup(self.temp.cleanup)
        self.root = Path(self.temp.name)
        self.home = self.root / 'home with spaces'
        self.bin = self.root / 'mocks'
        self.bin.mkdir()
        self.entry = self.home / '.local/bin/dust'
...
''')
    ...
    def fixture(self, system='Linux', machine='x86_64', prerelease=False,
        ...
        release = {'tag_name': 'v1.26.0', 'draft': False, 'prerelease': prerelease,
                   'assets': [{'name': name + '.tar.gz', 'digest': 'sha256:' + digest,
                    'browser_download_url': f'https://github.com/bootandy/dust/releases/download/v1.26.0/{name}.tar.gz'}]}
    ...
    def test_supported_platforms_and_repeat_install(self):
        ...
        backups = list((self.home / '.local/opt').glob('dust-entry-backup-*/dust'))
    ...
    def test_main_installer_dust_option(self):
        self.fixture()
        ...
        result = subprocess.run(['bash', str(SCRIPT.with_name('install-config.sh')), '--dust'],
    ...
    def test_existing_external_symlink_is_backed_up(self):
        self.fixture()
        vendor=self.root/'previous-dust'
