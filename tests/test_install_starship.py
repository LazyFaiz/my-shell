...
SCRIPT = Path(__file__).resolve().parents[1] / 'scripts/install-starship.sh'


class StarshipInstallerTests(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix='starship-installer-test-')
        self.addCleanup(self.temp.cleanup)
        self.root = Path(self.temp.name)
        self.home = self.root / 'home with spaces'
        self.bin = self.root / 'mocks'
        self.bin.mkdir()
        self.entry = self.home / '.local/bin/starship'
...
''')
    ...
    def fixture(self, system='Linux', machine='x86_64', prerelease=False,
        ...
        name = f'starship-{arch}-{platform}'
        program = b'#!/bin/sh\nexit 1\n' if broken_binary else b'#!/bin/sh\necho "starship v1.26.0"\n'
        ...
        release = {'tag_name': 'v1.26.0', 'draft': False, 'prerelease': prerelease,
                   'assets': [{'name': name + '.tar.gz', 'digest': 'sha256:' + digest,
                    'browser_download_url': f'https://github.com/starship/starship/releases/download/v1.26.0/{name}.tar.gz'}]}
    ...
    def test_supported_platforms_and_repeat_install(self):
        ...
        backups = list((self.home / '.local/opt').glob('starship-entry-backup-*/starship'))
    ...
    def test_main_installer_starship_option(self):
        self.fixture()
        ...
        result = subprocess.run(['bash', str(SCRIPT.with_name('install-config.sh')), '--starship'],
    ...
    def test_existing_external_symlink_is_backed_up(self):
        self.fixture()
        vendor=self.root/'previous-starship'
