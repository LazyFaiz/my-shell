...
SCRIPT = Path(__file__).resolve().parents[1] / 'scripts/install-fzf.sh'


class FzfInstallerTests(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix='fzf-installer-test-')
        self.addCleanup(self.temp.cleanup)
        self.root = Path(self.temp.name)
        self.home = self.root / 'home with spaces'
        self.bin = self.root / 'mocks'
        self.bin.mkdir()
        self.entry = self.home / '.local/bin/fzf'
...
''')
    ...
    def fixture(self, system='Linux', machine='x86_64', prerelease=False,
                bad_digest=False, broken_binary=False, missing_integration=False):
        self.executable('uname', '#!/bin/sh\ncase "$1" in\n-s) echo ' + system +
                        ';;\n-m) echo ' + machine + ';;\nesac\n')
        platform = 'linux' if system == 'Linux' else 'darwin'
        arch = 'arm64' if machine in ('aarch64', 'arm64') else 'amd64'
        name = f'fzf-0.74.4-{platform}_{arch}'
        ...
        release = {'tag_name': 'v0.74.4', 'draft': False, 'prerelease': prerelease,
                   'assets': [{'name': name + '.tar.gz', 'digest': 'sha256:' + digest,
                    'browser_download_url': f'https://github.com/junegunn/fzf/releases/download/v0.74.4/{name}.tar.gz'}]}
    ...
    def test_supported_platforms_and_repeat_install(self):
        ...
        backups = list((self.home / '.local/opt').glob('fzf-entry-backup-*/fzf'))
    ...
    def test_main_installer_fzf_option(self):
        self.fixture()
        ...
        result = subprocess.run(['bash', str(SCRIPT.with_name('install-config.sh')), '--fzf'],
    ...
    def test_missing_shell_integration_preserves_old_entry(self):
        self.fixture(missing_integration=True)
        result=self.run_installer()
        self.assertNotEqual(result.returncode,0)
        self.assertIn('step=verify-binary',result.stderr)
        self.assertEqual(self.entry.read_text(),'old entry\n')
